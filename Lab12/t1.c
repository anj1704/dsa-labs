...
struct Graph *get_graph(int V, int E) {
    ...
    for (int i = 0; i < V; i++) {
        // Initialize the allocated row to 0 (Do not visit all elements. Use the
        // <string.h> function bzero or memset)
    ...
    G->adjacency_list = (int **)malloc(sizeof(int *) * V);

    // Initialize all the linked lists to NULL (Will bzero or memset work here?
    // Why or why not?)
    for (int i = 0; i < V; i++) {
...
void add_edge_adj_list(struct Graph *G, int u, int v) {
    // Add an edge from u to v in the adjacency list. Use the ll_node structure
    // defined above
}
...
int main() {
    // You can change the file name to input1.txt or input2.txt to test your
    // code for different graphs
